 ...
 * 红黑树 map ，实现了 NavigableMap 的接口。
 * 如果 key 实现了 Comparable 接口，根据其 compareTo 方法进行排序
 * 否则根据外部的排序器（Comparator）进行排序
 *
 * <p>This implementation provides guaranteed log(n) time cost for the
 * {@code containsKey}, {@code get}, {@code put} and {@code remove}
 * operations.  Algorithms are adaptations of those in Cormen, Leiserson, and
 * Rivest's <em>Introduction to Algorithms</em>.
 * containsKey、get、put、remove 保证了时间复杂度是 log(n)。
 *
 ...
 * 线程不安全的，推荐使用 Collections#synchronizedSortedMap
 *
...
public class TreeMap<K, V>
    ...
    //3：找到自己应该挂在红黑树那个节点，通过比较大小即可
    //红黑树左边小，右边大，如果key比较相等的话，直接赋值原来的值
    //4：新建节点到 3 找到的父节点上
    //5：着色旋转
    public V put(K key, V value) {
        ...
        //cmp 代表最后一次对比的大小，大于 0 ，代表 e 在上一节点的右边，相等的情况已经处理了。
        // 相等的情况第二步已经处理了
        else {
            parent.right = e;
        }
