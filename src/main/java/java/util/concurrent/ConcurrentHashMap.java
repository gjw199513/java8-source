...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    //3:如果槽点是转发节点(正在扩容)，就会一直自旋等待扩容完成之后再新增，不是转移节点走 4；
    //4:槽点有值的，先锁定当前槽点，其余槽点不能操作
    //4.1 : 如果是链表，新增值到链表的尾部
    //4.2 : 如果是红黑树，在红黑树着色旋转的时候，会考虑把红黑树的根节点锁住
    //5:新增完成之后xcheck需不需要扩容，需要的话去扩容.
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            //如果当前索引位置没有值，直接创建
                ...
                //cas 在 i 位置创建新的元素，当i位置是空时，创建成功。结束for自循，否则继续自旋
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        //TreeBin 持有红黑树的引用，并且会对其加锁，保证其操作的线程安全
                            ...
                            binCount = 2;
                            //满足if的话，把老的值给oldVal
                            //在putTreeVal方法里面，再给红黑树重新着色旋转的时候
                            //会锁住红黑树的根节点
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            //CAS 赋值保证当前只有一个线程在初始化，-1代表当前只有一个线程能初始化
            //保证了数组的初始化的安全性
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                try {
                    //很有可能执行到这里的时候，table已经不为空了，这里是双重 check
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        for (int i = 0, bound = 0; ; ) {
            ...
            if (i < 0 || i >= n || i + n >= nextn) {
                int sc;
                // 拷贝结束，直接赋值，因为每次拷贝一个数字，都在原数组上放到转移节点
                // 原数组发现是转移节点，是不会操作的，会一直等待转移节点消失。
                // 也就是说数组节点一旦被标记为转移节点，是不会再发生任何变动的，
                // 所以不会有任何线程安全的问题
                if (finishing) {
                    nextTable = null;
                    table = nextTab;
